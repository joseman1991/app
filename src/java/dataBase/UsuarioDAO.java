...
public class UsuarioDAO extends ConexionMySQL {
    ...
    public void obtenerLista(Usuarios u) throws SQLException {
        ...
        while (resultado.next()) {
            ...
            user.setUsuario(resultado.getString(i++));
    ...
    public int insertarUsuario(Usuarios usuario) throws SQLException {
        abrirConexion();
        sentencia = conexion.prepareStatement("insert into usuarios(usuario,clave,nombre,apellido,idperfil) values (?,?,?,?,?)");
        int i = 1;
        sentencia.setString(i++, usuario.getUsuario());
    ...
    public int actualizarUsuario(Usuarios usuario) throws SQLException {
        abrirConexion();
        sentencia = conexion.prepareStatement("update usuarios set usuario=?,clave=?,nombre=?,apellido=?,idperfil=? where idusuario=?");
        int i = 1;
        sentencia.setString(i++, usuario.getUsuario());
        sentencia.setString(i++, usuario.getClave());
        sentencia.setString(i++, usuario.getNombre());
    ...
    public Usuarios obtenerUsusario(Usuarios user) throws SQLException {
        abrirConexion();
        sentencia = conexion.prepareStatement("select * from usuarios where  usuario=? and clave=?");
